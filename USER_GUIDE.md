...
- **File Geodatabase items (static uploads)** - These are user-uploaded FGDB files, not hosted Feature Services. When downloaded through the REST API, ArcGIS sometimes returns an HTML page containing a JavaScript redirect to cloud storage (AWS/Azure) instead of the actual file. A browser executes the JavaScript and completes the download, but the REST API client receives only the HTML page. These items may be downloadable manually from the item page even when the Backup Utility cannot retrieve them.

**How to verify:** For Feature Services, open the item's page in your browser and use Export Data > File Geodatabase. The Backup Utility uses the same REST API as that browser-based export. If the browser export also fails, the issue is with the item, not the backup. For static File Geodatabase items, try downloading the file directly from the item page - browser downloads may succeed where the REST API does not. (Note: ArcGIS Pro uses a different export method (arcpy) that may succeed where the REST API does not. The browser export is the correct comparison for Feature Services.)
